...
typedef struct
{
    ...
    int level_idc; // level
    int seq_parameter_set_id; // id
    int chroma_format_idc; // 420 or 444 or 422
    int residual_colour_transform_flag; // 是否使用残余颜色变换
...
typedef struct
{
    ...
    // 表明当前nal的重要性，值越大，越重要
    int nal_ref_idc;
    // 类型：1-普通slice，5-IDR，6-SEI，7-SPS、8-PPS
    int nal_unit_type;
    // SVC扩展（可分级编码）
