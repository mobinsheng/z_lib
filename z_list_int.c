...
#include <memory.h>
struct z_list_int_t{
    z_list_int_node_t* head;
    z_list_int_node_t* tail;
    size_t size;
};

z_list_int_t* z_list_int_create(){
    z_list_int_t* list = (z_list_int_t*)malloc(sizeof(z_list_int_t));
...
void z_list_int_free(z_list_int_t* list){
    z_list_int_node_t* node = list->head;
    while (node) {
        z_list_int_node_t* next = node->next;
...
z_list_int_node_t* z_list_int_pushback(z_list_int_t* list,int64_t data){
    z_list_int_node_t* node = (z_list_int_node_t*)malloc(sizeof(z_list_int_node_t));
    memcpy(&node->int_val,&data,sizeof(data));
...
void z_list_int_popback(z_list_int_t* list){
    z_list_int_node_t* node = list->tail;
    z_list_int_remove(list,node); // 内部有对size的操作
}
...
z_list_int_node_t* z_list_int_insert_after(z_list_int_t* list,z_list_int_node_t* pos,int64_t data){
    if(pos->next == NULL){
        return z_list_int_pushback(list,data); // 内部有对size的操作
    }
    ...
    // 记录pos的下一个节点
    z_list_int_node_t* org_next = pos->next;
...
z_list_int_node_t* z_list_int_insert_before(z_list_int_t* list,z_list_int_node_t* pos,int64_t data){
    if(pos->prev == NULL){
        return z_list_int_pushfront(list,data); // 内部有对size的操作
    }
    ...
    z_list_int_node_t* org_prev = pos->prev;
...
void z_list_int_remove(z_list_int_t* list,z_list_int_node_t* node){
    z_list_int_node_t* org_prev = node->prev;
    z_list_int_node_t* org_next = node->next;
