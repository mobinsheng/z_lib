...
z_list_t* z_list_create(){
    z_list_t* list = (z_list_t*)malloc(sizeof(z_list_t));
...
void z_list_free(z_list_t* list){
    z_list_node_t* node = list->head;
    while (node) {
        z_list_node_t* next = node->next;
...
z_list_node_t* z_list_pushback(z_list_t* list,void* data){
    z_list_node_t* node = (z_list_node_t*)malloc(sizeof(z_list_node_t));
    memcpy(&node->ptr_val,&data,sizeof(data));
...
void z_list_popback(z_list_t* list){
    z_list_node_t* node = list->tail;
    z_list_remove(list,node); // 内部有对size的操作
}
...
z_list_node_t* z_list_insert_after(z_list_t* list,z_list_node_t* pos,void* data){
    if(pos->next == NULL){
        return z_list_pushback(list,data); // 内部有对size的操作
    }
    ...
    // 记录pos的下一个节点
    z_list_node_t* org_next = pos->next;
...
z_list_node_t* z_list_insert_before(z_list_t* list,z_list_node_t* pos,void* data){
    if(pos->prev == NULL){
        return z_list_pushfront(list,data); // 内部有对size的操作
    }
    ...
    z_list_node_t* org_prev = pos->prev;
...
void z_list_remove(z_list_t* list,z_list_node_t* node){
    z_list_node_t* org_prev = node->prev;
    z_list_node_t* org_next = node->next;
