...
static z_rtmp_result_t rtmp_packet_amf_analyze(
    z_rtmp_packet_t *packet,
    unsigned char *amf_packets_buffer, size_t rtmp_body_size);
static z_rtmp_result_t rtmp_packet_process_body(
    z_rtmp_packet_t *packet, unsigned char *body_buffer, size_t body_size);
static z_rtmp_result_t z_rtmp_packet_serialize_amf(
    z_rtmp_packet_t *packet,
...
static z_rtmp_result_t z_rtmp_packet_serialize_data(
    z_rtmp_packet_t *packet,
    size_t *total_serialized_size,
...
z_rtmp_packet_t *z_rtmp_packet_create(void)
{
    z_rtmp_packet_t *packet;

    packet = (z_rtmp_packet_t*)malloc(sizeof(z_rtmp_packet_t));
    packet->inner_amf_packets = NULL;
    packet->body_data = NULL;
    z_rtmp_packet_cleanup(packet);

    return packet;
}


void z_rtmp_packet_cleanup(z_rtmp_packet_t *packet)
{
    ...
    packet->object_id = 0;
    packet->timer = 0;
    packet->data_type = (z_rtmp_datatype_t)0;
    packet->stream_id = 0;
    inner_amf = packet->inner_amf_packets;
    while (inner_amf) {
        next = inner_amf->next;
        z_amf_packet_free(inner_amf->amf);
        free(inner_amf);
        inner_amf = next;
    }
    packet->inner_amf_packets = NULL;
...
z_rtmp_result_t z_rtmp_packet_analyze_data(
    z_rtmp_packet_t *packet,
    ...
    size_t *packet_size)
{
    ...
    packet->timer = z_read_be24int(data + 1);
#ifdef DEBUG
    printf("timer: %ld\n", packet->timer);
#endif
    ...
    rtmp_body_size = z_read_be24int(data + 4);
#ifdef DEBUG
...
#endif
    packet->data_type = (z_rtmp_datatype_t)data[7];
#ifdef DEBUG
    ...
    if (header_size_magic == HEADER_MAGIC_08) {
        ...
        header_size = 12;
        packet->stream_id = z_read_le32int(data + 8);
        if (rtmp_body_size == 0) {
            *packet_size = 12;
            return RTMP_SUCCESS;
        }
...
z_rtmp_result_t rtmp_packet_process_body(
    z_rtmp_packet_t *packet, unsigned char *body_buffer, size_t body_size)
{
    z_rtmp_result_t amf_ret;
...
z_rtmp_result_t rtmp_packet_amf_analyze(
    z_rtmp_packet_t *packet,
    unsigned char *amf_buffer, size_t amf_buffer_size)
{
    ...
    z_amf_packet_t *amf;
    z_rtmp_packet_inner_amf_t *inner_amf;
    z_rtmp_packet_inner_amf_t *prev_inner_amf;
    size_t analyzed_amf_packet_size;

    buffer_position = 0;
    packet->inner_amf_packets = NULL;
    while (buffer_position < amf_buffer_size) {
        inner_amf = (z_rtmp_packet_inner_amf_t*)malloc(
            sizeof(z_rtmp_packet_inner_amf_t));
        if (inner_amf == NULL) {
            return RTMP_ERROR_MEMORY_ALLOCATION;
        }
        amf = z_amf_packet_analyze_data(
...
z_rtmp_result_t z_rtmp_packet_serialize(
    z_rtmp_packet_t *packet,
    unsigned char *output_buffer, size_t output_buffer_size,
    size_t amf_chunk_size,
    size_t *packet_size)
{
    ...
    z_write_be24int(output_buffer + 1, packet->timer);

    if (packet->body_type == RTMP_BODY_TYPE_AMF) {
        amf_size = 0;
        inner_amf = packet->inner_amf_packets;
        while (inner_amf) {
            amf_size += z_amf_packet_get_size(inner_amf->amf);
            inner_amf = inner_amf->next;
        }
        z_write_be24int(output_buffer + 4, (int)amf_size);
    } else if (packet->body_type == RTMP_BODY_TYPE_DATA) {
        amf_size = 0;
        z_write_be24int(output_buffer + 4, (int)packet->body_data_length);
    }
    ...
    z_write_le32int(output_buffer + 8, packet->stream_id);
    total_serialized_size = header_size;

    if (packet->body_type == RTMP_BODY_TYPE_AMF) {
        result = z_rtmp_packet_serialize_amf(
            packet,
        ...
        result = z_rtmp_packet_serialize_data(
            packet,
            &total_serialized_size,
...
z_rtmp_result_t z_rtmp_packet_serialize_amf(
    z_rtmp_packet_t *packet,
    size_t *total_serialized_size,
    size_t amf_size, size_t amf_chunk_size, size_t header_size,
    unsigned char *output_buffer, size_t output_buffer_size)
{
    ...
    inner_amf = packet->inner_amf_packets;
    while (inner_amf) {
        serialized_amf_size = z_amf_packet_serialize(
            inner_amf->amf,
...
z_rtmp_result_t z_rtmp_packet_serialize_data(
    z_rtmp_packet_t *packet,
    size_t *total_serialized_size,
    size_t amf_chunk_size, size_t header_size,
    unsigned char *output_buffer, size_t output_buffer_size)
...
void z_rtmp_packet_free(z_rtmp_packet_t *packet)
{
    z_rtmp_packet_inner_amf_t *inner_amf;
    z_rtmp_packet_inner_amf_t *next;
...
z_rtmp_result_t z_rtmp_packet_add_amf(
    z_rtmp_packet_t *packet,
    z_amf_packet_t *amf)
{
    z_rtmp_packet_inner_amf_t *inner_amf;
    z_rtmp_packet_inner_amf_t *last_inner_amf;
...
z_rtmp_result_t z_rtmp_packet_allocate_body_data(
...
void z_rtmp_packet_retrieve_status_info(
